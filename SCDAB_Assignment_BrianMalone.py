...
sg.popup('Welcome to the Framingham Health Screening Model', 'Press OK & Select your home directory')

#20210720 Using tkinter to select the home directory to be used in the code.
root = Tk()
root.withdraw()
...
def covar():
    ...
    layout = [[sg.Text('Calculate Pearson Covariance two values: (Put any two values into the input boxes)')],
              [sg.Text('education \ncurrentSmoker \nCigsPerDay \nBPMeds \nBMI \nsysBP \ndiaBP \n male'), sg.InputText()],
              [sg.Text( 'prevalentHyp \ndiabetes \ntotChol \nprevalentStroke \nHeartRate \nglucose \n age'), sg.InputText()], [sg.Submit(), sg.Cancel()]]
    window = sg.Window('Pearson Covariance Inputs', layout, size=(550, 340))
    while True:
        ...
        sg.popup_ok('Covariance is  ' + cov + '.', title='Output')
...
def SeparateMaleFemale():
    ...
    if event == 'Yes':
        with open('FrameComplete.csv', newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            #print(val) # Showing all data in a dictionary
            m = open('MaleContacts.csv', "w")
            f = open('femaleContacts.csv', "w")

            fieldnames = ('Phone', 'GivenName')
            ...
            for row in reader:
                            if row['Gender_x'] == 'male':
                                writer.writerow({'Phone': row['Phone'], 'GivenName': row['GivenName']})
                            else:
                                writerf.writerow({'Phone': row['Phone'], 'GivenName': row['GivenName']})
    else:
...
def rate(file,column):
    with open(file) as csvfile:
        ...
        #test = input("Enter Rate: ")
        # GUI to take input from User
        test = easygui.enterbox("Above what value do you want output for?:")
...
#202106 Joining the dataframe with ref and phone numbers to the dataframe with ref and health details
df3=pd.merge(df,df2,on="Phone")
#Function to clean the dataframe of null values
clean(df3)
...
window = sg.Window('Tree Model Properties', layout) # Name of the Box
